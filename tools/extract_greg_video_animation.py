#!/usr/bin/env python3
"""Extract game-ready transparent Greg attacks and idle from green-screen MP4 files.
...
SOURCE_ROOT = ROOT / "source_art" / "animations" / "greg"
OUTPUT_ROOT = ROOT / "assets" / "characters" / "greg"
MANIFEST_PATH = OUTPUT_ROOT / "video_attacks_manifest.json"
CANVAS = (1024, 1024)
SAFE_PADDING_RATIO = 0.16
DESIRED_GAME_HEIGHT = 290.0
GROUND_LOCKED_ANIMATIONS = {"idle_video", "hit_video", "death_video"}
SIZE_NORMALIZED_ANIMATIONS = {"idle_video", "hit_video", "death_video"}
FRAME_SIZE_NORMALIZED_ANIMATIONS = {"hit_video"}
...
ATTACKS: dict[str, AttackSpec] = {
    "idle_video": AttackSpec(
        "greg_idle_source_v2.mp4",
        tuple(range(16, 121, 2)),
        12.0,
        0,
    ),
    "hit_video": AttackSpec(
        "greg_hit_source.mp4",
        (15, 18, 21, 24, 27, 30, 33, 36),
        15.0,
        0,
    ),
    "death_video": AttackSpec(
        "greg_death_source.mp4",
        (0, 4, 8, 12, 18, 24, 30, 36, 42, 48, 52, 56, 62, 70, 80),
        12.0,
        0,
    ),
    "attack_01": AttackSpec(
        "greg_attack_01_source.mp4",
        (0, 3, 6, 9, 12, 15, 18, 21, 24, 26),
        12.0,
        4,
    ),
    "attack_02": AttackSpec(
        "greg_attack_02_source.mp4",
        (0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 29),
        12.0,
        5,
    ),
    "attack_03": AttackSpec(
        "greg_attack_03_source.mp4",
        (0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33),
        12.0,
        7,
    ),
    "attack_04": AttackSpec(
        "greg_attack_04_source.mp4",
...
def decode_selected_frames(
    ...
    with tempfile.TemporaryDirectory(prefix="greg-key-") as temporary:
...
def prepare_all() -> dict:
    ...
    for attack_name, spec in ATTACKS.items():
        ...
        first_anchor = reference_anchor(keyed[spec.selected_frames[0]])
        first_box = content_bbox(keyed[spec.selected_frames[0]])
        first_content_height = float(first_box[3] - first_box[1])
        ...
        decoded[attack_name] = {
            ...
            "frames": keyed,
            "source_anchor": (first_anchor[0], first_anchor[1]),
            "reference_height": first_anchor[2],
            "content_height": first_content_height,
            "size_scale": 1.0,
            "frame_anchors": {},
            "frame_size_scales": {},
        }

    canonical_reference_height = float(
        np.median(
            [
                attack["reference_height"]
                for attack_name, attack in decoded.items()
                if attack_name not in SIZE_NORMALIZED_ANIMATIONS
            ]
        )
    )
    canonical_content_height = float(
        np.median(
            [
                attack["content_height"]
                for attack_name, attack in decoded.items()
                if attack_name not in SIZE_NORMALIZED_ANIMATIONS
            ]
        )
    ...
    for attack_name, attack in decoded.items():
        if attack_name in SIZE_NORMALIZED_ANIMATIONS:
            attack["size_scale"] = canonical_content_height / attack["content_height"]

    source_anchor = (
        float(np.median([item[0] for item in first_anchors])),
        float(np.median([item[1] for item in first_anchors])),
    )
    reference_height = canonical_reference_height

    relative_boxes: list[tuple[float, float, float, float]] = []
    for attack_name, attack in decoded.items():
        clip_anchor = attack["source_anchor"]
        for source_index, frame in attack["frames"].items():
            frame_reference = reference_anchor(frame)
            frame_anchor = (
                (frame_reference[0], frame_reference[1])
                if attack_name in GROUND_LOCKED_ANIMATIONS
                else clip_anchor
            )
            attack["frame_anchors"][source_index] = frame_anchor
            left, top, right, bottom = content_bbox(frame)
            frame_size_scale = attack["size_scale"]
            if attack_name in FRAME_SIZE_NORMALIZED_ANIMATIONS:
                frame_size_scale = canonical_content_height / float(bottom - top)
            attack["frame_size_scales"][source_index] = frame_size_scale
            relative_boxes.append(
                (
                    (left - frame_anchor[0]) * frame_size_scale,
                    (top - frame_anchor[1]) * frame_size_scale,
                    (right - frame_anchor[0]) * frame_size_scale,
                    (bottom - frame_anchor[1]) * frame_size_scale,
                )
            )
    ...
    for attack_name, attack in decoded.items():
        ...
        for output_index, source_index in enumerate(spec.selected_frames, start=1):
            rendered = render_frame(
                attack["frames"][source_index],
                attack["frame_anchors"][source_index],
                canvas_anchor,
                extraction_scale * attack["frame_size_scales"][source_index],
            )
            file_name = f"greg_{attack_name}_{output_index:03d}.png"
            ...
            records.append(
                {
                    ...
                    "size_scale": round(attack["frame_size_scales"][source_index], 6),
        ...
        manifest["attacks"][attack_name] = {
            ...
            "source_anchor": [round(value, 3) for value in attack["source_anchor"]],
            "ground_locked": attack_name in GROUND_LOCKED_ANIMATIONS,
            "size_normalized": attack_name in SIZE_NORMALIZED_ANIMATIONS,
            "source_reference_height": round(attack["reference_height"], 3),
            "source_content_height": round(attack["content_height"], 3),
            "size_scale": round(attack["size_scale"], 6),
            "game_fps": spec.fps,
...
def validate() -> list[str]:
    ...
    for attack_name, attack in manifest["attacks"].items():
        minimum_frames = 12 if attack_name == "idle_video" else 6
        maximum_frames = 72 if attack_name == "idle_video" else 15
        if not (minimum_frames <= len(attack["frames"]) <= maximum_frames):
...
def main() -> int:
    ...
    # Shared alignment is calculated across the idle and all four attacks even when one is
