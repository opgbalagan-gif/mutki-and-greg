...
func _run() -> void:
	...
	for fighter_id in ["greg", "mutki"]:
		...
		var expected_helper: String = fighter_id
		var expected_title := "СУПЕРУДАР ГРИШИ" if fighter_id == "greg" else "СУПЕРУДАР МУТКИ"
		check(game.hud.special_name == expected_title, "super title identifies the fighter")
		var portrait := game.hud.fighter_portrait.texture as AtlasTexture
		check((portrait.region.position.x == 410) == (expected_helper == "mutki"), "super avatar identifies the fighter")
		...
		if not ResourceLoader.exists(AssistVideoPlayer.VIDEO_PATHS[expected_helper]):
			check(not missing.play_helper(expected_helper) and not paused, "not-yet-added fighter clip leaves fallback available")
		missing.queue_free()
		...
		check(player.helper_requested == expected_helper, "button requests the fighter's video")
		...
		check(game.arena_assist.playing and impacts.count == 0, "arena move precedes damage")
		...
		check(impacts.count == 1, "super applies exactly one impact")
		player._finish()
	...
	print("ASSIST_VIDEO_PASS: fighter/title/avatar/button/pause/no-duplicate/natural-finish/no-skip/no-escape/resume/cleanup" if failures.is_empty() else "ASSIST_VIDEO_FAIL")
